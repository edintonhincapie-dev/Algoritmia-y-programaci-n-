"""
pf_Algoritmos - PrestaFácil: Gestor de Préstamos (Versión Procedural)
...
def validar_tiempo(tiempo):
    ...
    if tiempo not in TIEMPOS_PERMITIDOS:
...
def cargar_usuarios():
    """pf_Algoritmos - Carga la lista de usuarios desde usuarios.json."""
    return _cargar_json(RUTA_USUARIOS)
 
 
def guardar_usuarios(usuarios):
    """pf_Algoritmos - Guarda la lista de usuarios en usuarios.json."""
    _guardar_json(RUTA_USUARIOS, usuarios)
 
 
def cargar_prestamos():
    """pf_Algoritmos - Carga la lista de préstamos/ítems desde items.json."""
    return _cargar_json(RUTA_PRESTAMOS)
 
 
def guardar_prestamos(prestamos):
    """pf_Algoritmos - Guarda la lista de préstamos/ítems en items.json."""
    _guardar_json(RUTA_PRESTAMOS, prestamos)
 
 
def cargar_admins():
    """pf_Algoritmos - Carga (o inicializa) las credenciales de administrador."""
...
def exportar_csv():
    """pf_Algoritmos - Exporta préstamos a CSV."""
    prestamos = cargar_prestamos()
    ruta_csv = os.path.join(DIRECTORIO, "data", "prestamos_export.csv")
    campos = ["id_item", "nombre_item", "categoria", "precio", "estado",
              "documento_usuario", "fecha_prestamo", "activo", "fecha_devolucion"]
...
def registrar_usuario():
    ...
    while True:
        nombre = pedir("Nombre")
        ok_val, msg = validar_nombre(nombre)
        if ok_val: break
        error(msg)
    ...
    while True:
        ...
        ok_val, msg = validar_nombre(apellido)
        if ok_val: break
        error(msg)
    ...
    while True:
        ...
        ok_val, msg = validar_documento(doc)
        if ok_val: break
        error(msg)
    ...
    while True:
        ...
        ok_val, msg = validar_correo(correo)
        if ok_val: break
        error(msg)
 
    # Tiempo de préstamo
    while True:
        info(f"Tiempos permitidos: {TIEMPOS_PERMITIDOS} días")
        entrada = pedir("Tiempo de préstamo (días)")
        if entrada.isdigit():
            tiempo = int(entrada)
            ok_val, msg = validar_tiempo(tiempo)
            if ok_val: break
            error(msg)
        else:
...
def registrar_item():
    ...
    # Categoría
    print()
    for k, v in CATEGORIAS_NOMBRES.items():
        print(f"  {k}. {v}")
    while True:
    ...
    for k, (nombre_estado, valor) in ESTADOS_FUZZY.items():
        print(f"  {k}. {nombre_estado} (μ={valor})")
    while True:
        est = pedir("Seleccione estado")
        if est in ESTADOS_FUZZY:
            break
        error("Estado inválido.")
    estado_nombre = ESTADOS_FUZZY[est][0]
 
    # Generar ID
    id_item = generar_id(cat)
    ...
    nuevo_item = {
        "id_item": id_item,
        "nombre_item": nombre,
        "categoria": CATEGORIAS_NOMBRES[cat],
        "precio": precio,
...
def registrar_devolucion():
    ...
    separador()
    for p in activos:
        print(f"  {p['id_item']:<12} {p['nombre_item']:<20} {p['fecha_prestamo']:<16} {dias_prestado(p)} días")
    separador()
    ...
    # Guardamos los datos ANTES de modificar el diccionario, para el certificado
    fecha_prestamo_original = item_dev["fecha_prestamo"]
    dias_totales = dias_prestado(item_dev)
 
    for p in prestamos:
        if p["id_item"] == id_dev:
            p["activo"] = False
            p["fecha_devolucion"] = hoy
            p["documento_usuario"] = ""   # libera el ítem para volver a prestarlo
            p["fecha_prestamo"] = ""      # limpia la fecha del préstamo anterior
            break
    guardar_prestamos(prestamos)
    ...
    with open(ruta_cert, "w", encoding="utf-8") as f:
        ...
        f.write(f"Fecha de préstamo : {fecha_prestamo_original}\n")
        f.write(f"Fecha devolución  : {hoy}\n")
        f.write(f"Días prestado     : {dias_totales}\n\n")
...
def consultar_vencidos():
    ...
    vencidos = [p for p in prestamos if p["activo"] and dias_prestado(p) > 30]
    if not vencidos:
    ...
    for p in vencidos:
        ...
        print(f"  {AMARILLO}► {p['nombre_item']} ({p['id_item']}) — {dias_prestado(p)} días — {nombre_u}{RESET}")
        ...
        with open(ruta_fact, "w", encoding="utf-8") as f:
            ...
            f.write(f"Días prestado     : {dias_prestado(p)}\n\n")
...
def consultar_prestados():
    ...
    activos.sort(key=lambda x: dias_prestado(x), reverse=True)
    print(f"  {'#':<4} {'ID':<12} {'Ítem':<20} {'Días':>6}  {'Estado':<10} {'Usuario'}")
    separador()
    for i, p in enumerate(activos, 1):
        usuario = next((u for u in usuarios if u["documento"] == p["documento_usuario"]), None)
        nombre_u = f"{usuario['nombre']} {usuario['apellido']}" if usuario else p["documento_usuario"]
        dias = dias_prestado(p)
        if dias > 30:
            alerta = f"{ROJO}⚠ VENCIDO{RESET}"
        elif dias > 20:
            alerta = f"{AMARILLO}⚡ PRONTO{RESET}"
        else:
            alerta = ""
        print(f"  {i:<4} {p['id_item']:<12} {p['nombre_item']:<20} {dias:>6}  {p['estado']:<10} {nombre_u} {alerta}")
...
def modulo_administrador():
    ...
    while True:
        ...
        if op == "1":
            ...
            ventas = [p for p in prestamos if p["activo"] and dias_prestado(p) > 30]
            ok(f"Total ventas generadas: {len(ventas)}")
 
        elif op == "4":
            total_pago = sum(p["precio"] * 1.23 for p in prestamos if p["activo"] and dias_prestado(p) > 30)
